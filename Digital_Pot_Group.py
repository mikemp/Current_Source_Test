...
class Digital_Pot_Group:
    ...
    def set_wiper_pos(self,pos):
        num_pots_full = (pos-self.num_pots)%(self.num_pots-1)
        set_remain = (pos-self.num_pots)/(self.num_pots-1)
        ...
        if pos <= self.total_positions and pos > self.num_pots:
            ...
            if set_remain > 0:
                self.variable_R_array[num_pots_full].set_wiper_pos(set_remain)
